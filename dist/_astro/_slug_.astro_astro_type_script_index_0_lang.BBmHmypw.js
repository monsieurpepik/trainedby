const R="https://mezhtdbfyvkshpuplqqw.supabase.co",A="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1lemh0ZGJmeXZrc2hwdXBscXF3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ5MzE4NDIsImV4cCI6MjA5MDUwNzg0Mn0.zJG9xodJS70Wl2IJWiLxk2bSL7eukg5uUbLfF7jvQAo";function P(){if(window.__TRAINER_SLUG__)return window.__TRAINER_SLUG__;const e=location.pathname.replace(/^\//,"").trim();if(e&&e!=="index.html"&&e!=="profile")return e;const o=new URLSearchParams(location.search);return o.get("trainer")||o.get("slug")||""}async function U(){const e=P();if(!e){M("No trainer specified");return}try{const o=await fetch(`${R}/functions/v1/get-trainer?slug=${encodeURIComponent(e)}`,{headers:{Authorization:`Bearer ${A}`,apikey:A}});if(!o.ok)throw new Error(`HTTP ${o.status}`);const r=await o.json();if(!r||r.error)throw new Error("Trainer not found");const a=r.trainer||r,n=r.packages||[];N(a,n)}catch{M("Trainer not found")}}function M(e="Trainer not found"){const o=document.getElementById("profile-mount"),r=document.getElementById("loading-state");r&&(r.style.display="none"),o&&(o.style.display="block",o.innerHTML=`
          ...
          </div>`)}function L(e){const o=(e||"?").split(" ").map(x=>x[0]||"").join("").toUpperCase().slice(0,2)||"?",r=getComputedStyle(document.documentElement).getPropertyValue("--brand").trim()||"#FF5C00",a=parseInt(r.slice(1,3),16)||255,n=parseInt(r.slice(3,5),16)||92,s=parseInt(r.slice(5,7),16)||0,d=document.createElement("canvas");d.width=800,d.height=480;const t=d.getContext("2d");t.fillStyle="#080808",t.fillRect(0,0,800,480);const g=t.createLinearGradient(0,0,800,480);g.addColorStop(0,`rgba(${a},${n},${s},0.40)`),g.addColorStop(.6,`rgba(${Math.round(a*.5)},${Math.round(n*.5)},${Math.round(s*.5)},0.15)`),g.addColorStop(1,"rgba(8,8,8,0.95)"),t.fillStyle=g,t.fillRect(0,0,800,480);const p=t.createRadialGradient(400,200,0,400,200,180);return p.addColorStop(0,`rgba(${a},${n},${s},0.22)`),p.addColorStop(1,"rgba(0,0,0,0)"),t.fillStyle=p,t.fillRect(0,0,800,480),t.beginPath(),t.arc(400,200,92,0,Math.PI*2),t.strokeStyle=`rgba(${a},${n},${s},0.55)`,t.lineWidth=2,t.stroke(),t.fillStyle="#ffffff",t.font="bold 76px Manrope, system-ui, sans-serif",t.textAlign="center",t.textBaseline="middle",t.fillText(o,400,200),d.toDataURL("image/png")}function S(e){const o=document.getElementById("bg");if(o){const a=L(e);o.style.backgroundImage=`url('${a}')`,o.style.backgroundSize="cover"}const r=document.querySelector("#profile-mount img[data-hero]");r&&(r.style.display="none")}function N(e,o){const r=document.getElementById("loading-state"),a=document.getElementById("profile-mount");if(r&&(r.style.display="none"),!a)return;a.style.display="block";const n=e.name||e.full_name||"",s=e.avatar_url||e.profile_photo_url||"",d=document.getElementById("bg");if(s){const i=new Image;i.onload=()=>{d&&(d.style.backgroundImage=`url('${s}')`)},i.onerror=()=>S(n),i.src=s}else setTimeout(()=>S(n),50);if(s){const i=new Image;i.crossOrigin="anonymous",i.onload=function(){try{const y=document.createElement("canvas");y.width=8,y.height=8;const z=y.getContext("2d");z.drawImage(i,0,0,8,8);const c=z.getImageData(0,0,8,8).data;let u=0,m=0,h=0,f=0;for(let l=0;l<c.length;l+=4){const C=(c[l]+c[l+1]+c[l+2])/3;C>30&&C<230&&(u+=c[l],m+=c[l+1],h+=c[l+2],f++)}f>0&&(u=Math.round(u/f),m=Math.round(m/f),h=Math.round(h/f),document.documentElement.style.setProperty("--avatar-color",`${u},${m},${h}`))}catch{}},i.src=s}const t=window.__BRAND__&&window.__BRAND__.name?window.__BRAND__.name:window.__BRAND__||"TrainedBy",g=Array.isArray(e.specialties)?e.specialties:e.specialties?[e.specialties]:["Personal Trainer"],p=e.avg_rating?parseFloat(e.avg_rating).toFixed(1):"5.0",x=e.review_count||0,B=e.plan==="pro"||e.plan==="elite",F=e.reps_verified||e.is_verified||e.verification_status==="verified",E=e.city||"",T=e.country||"",v=[E,T].filter(Boolean).join(", "),w=e.bio||"",$=e.instagram||e.instagram_handle||"",k=e.whatsapp||e.phone||"",I=Array.isArray(e.certifications)?e.certifications:[],b=Array.isArray(o)?o:[],_=b[0]?.price||null,j=b[0]?.currency||"";document.title=`${n}  -  Verified Trainer on ${t}`,a.innerHTML=`
        <div style="max-width:680px;margin:0 auto;padding:24px 16px 80px">
          ...
          <div style="background:rgba(10,10,10,0.72);backdrop-filter:blur(28px);-webkit-backdrop-filter:blur(28px);border:1px solid rgba(255,255,255,0.10);border-radius:24px;overflow:hidden;margin-bottom:16px;box-shadow:0 8px 48px rgba(0,0,0,0.5)">
            <div style="position:relative;height:240px;background:linear-gradient(135deg,#1a1a1a,#0a0a0a)">
              ...
              ${F?'<div style="position:absolute;top:16px;right:16px;background:rgba(34,197,94,0.18);border:1px solid rgba(34,197,94,0.35);color:#22c55e;padding:5px 12px;border-radius:20px;font-size:11px;font-weight:700;backdrop-filter:blur(12px);letter-spacing:0.03em">✓ Verified</div>':""}
              ${B?'<div style="position:absolute;top:16px;left:16px;background:linear-gradient(135deg,rgba(197,162,39,0.25),rgba(245,208,107,0.15));border:1px solid rgba(197,162,39,0.4);color:#f5d06b;padding:5px 12px;border-radius:20px;font-size:11px;font-weight:800;backdrop-filter:blur(12px);letter-spacing:0.05em">PRO</div>':""}
            </div>
            <div style="padding:22px">
              <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap">
                ...
                <div style="text-align:right">
                  <div style="font-family:var(--font-display,'Manrope',sans-serif);font-size:22px;font-weight:900;color:var(--brand,#FF5C00);letter-spacing:-0.02em">${_?`From ${j} ${_}`:""}</div>
          ...
          <div style="background:rgba(10,10,10,0.65);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);border:1px solid rgba(255,255,255,0.09);border-radius:20px;padding:20px;margin-bottom:16px;box-shadow:0 4px 24px rgba(0,0,0,0.3)">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:8px">
              ${I.map(i=>`<span style="background:rgba(34,197,94,0.1);border:1px solid rgba(34,197,94,0.3);color:#22c55e;padding:6px 14px;border-radius:20px;font-size:13px;font-weight:600">${i}</span>`).join("")}
            </div>
          </div>`:""}
