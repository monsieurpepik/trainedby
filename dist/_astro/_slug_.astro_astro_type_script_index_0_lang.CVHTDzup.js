const P="https://mezhtdbfyvkshpuplqqw.supabase.co",B="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1lemh0ZGJmeXZrc2hwdXBscXF3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ5MzE4NDIsImV4cCI6MjA5MDUwNzg0Mn0.zJG9xodJS70Wl2IJWiLxk2bSL7eukg5uUbLfF7jvQAo";function R(){if(window.__TRAINER_SLUG__)return window.__TRAINER_SLUG__;const e=location.pathname.replace(/^\//,"").trim();if(e&&e!=="index.html"&&e!=="profile")return e;const i=new URLSearchParams(location.search);return i.get("trainer")||i.get("slug")||""}async function S(){const e=R();if(!e){A("No trainer specified");return}try{const i=await fetch(`${P}/functions/v1/get-trainer?slug=${encodeURIComponent(e)}`,{headers:{Authorization:`Bearer ${B}`,apikey:B}});if(!i.ok)throw new Error(`HTTP ${i.status}`);const r=await i.json();if(!r||r.error)throw new Error("Trainer not found");const d=r.trainer||r,n=r.packages||[];L(d,n)}catch{A("Trainer not found")}}function A(e="Trainer not found"){const i=document.getElementById("profile-mount"),r=document.getElementById("loading-state");r&&(r.style.display="none"),i&&(i.style.display="block",i.innerHTML=`
          ...
          </div>`)}function L(e,i){const r=document.getElementById("loading-state"),d=document.getElementById("profile-mount");if(r&&(r.style.display="none"),!d)return;d.style.display="block";const n=e.name||e.full_name||"",a=e.avatar_url||e.profile_photo_url||"",u=document.getElementById("bg");if(u&&a){const t=new Image;t.onload=()=>{u.style.backgroundImage=`url('${a}')`},t.onerror=()=>{},t.src=a}if(a){const t=new Image;t.crossOrigin="anonymous",t.onload=function(){try{const c=document.createElement("canvas");c.width=8,c.height=8;const z=c.getContext("2d");z.drawImage(t,0,0,8,8);const s=z.getImageData(0,0,8,8).data;let g=0,f=0,x=0,l=0;for(let o=0;o<s.length;o+=4){const I=(s[o]+s[o+1]+s[o+2])/3;I>30&&I<230&&(g+=s[o],f+=s[o+1],x+=s[o+2],l++)}l>0&&(g=Math.round(g/l),f=Math.round(f/l),x=Math.round(x/l),document.documentElement.style.setProperty("--avatar-color",`${g},${f},${x}`))}catch{}},t.src=a}const b=window.__BRAND__&&window.__BRAND__.name?window.__BRAND__.name:window.__BRAND__||"TrainedBy",C=Array.isArray(e.specialties)?e.specialties:e.specialties?[e.specialties]:["Personal Trainer"],y=e.avg_rating?parseFloat(e.avg_rating).toFixed(1):"5.0",m=e.review_count||0,M=e.plan==="pro"||e.plan==="elite",j=e.reps_verified||e.is_verified||e.verification_status==="verified",T=e.city||"",E=e.country||"",h=[T,E].filter(Boolean).join(", "),v=e.bio||"",w=e.instagram||e.instagram_handle||"",k=e.whatsapp||e.phone||"",$=Array.isArray(e.certifications)?e.certifications:[],p=Array.isArray(i)?i:[],_=p[0]?.price||null,F=p[0]?.currency||"";document.title=`${n}  -  Verified Trainer on ${b}`,d.innerHTML=`
        <div style="max-width:680px;margin:0 auto;padding:24px 16px 80px">
          ...
          <div style="background:rgba(10,10,10,0.72);backdrop-filter:blur(28px);-webkit-backdrop-filter:blur(28px);border:1px solid rgba(255,255,255,0.10);border-radius:24px;overflow:hidden;margin-bottom:16px;box-shadow:0 8px 48px rgba(0,0,0,0.5)">
            <div style="position:relative;height:240px;background:linear-gradient(135deg,#1a1a1a,#0a0a0a)">
              ${a?`<img src="${a}" alt="${n}" style="width:100%;height:100%;object-fit:cover;object-position:top" onerror="this.style.display='none'">`:""}
          ...
          <div style="background:rgba(10,10,10,0.65);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);border:1px solid rgba(255,255,255,0.09);border-radius:20px;padding:20px;margin-bottom:16px;box-shadow:0 4px 24px rgba(0,0,0,0.3)">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:8px">
              ${$.map(t=>`<span style="background:rgba(34,197,94,0.1);border:1px solid rgba(34,197,94,0.3);color:#22c55e;padding:6px 14px;border-radius:20px;font-size:13px;font-weight:600">${t}</span>`).join("")}
            </div>
          </div>`:""}
          ...
          <div style="background:rgba(10,10,10,0.65);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);border:1px solid rgba(255,255,255,0.09);border-radius:20px;padding:20px;margin-bottom:16px;box-shadow:0 4px 24px rgba(0,0,0,0.3)">
            ...
            <div style="display:flex;flex-direction:column;gap:10px">
              ...
              <div style="display:flex;align-items:center;justify-content:space-between;padding:14px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:12px">
                <div>
                  <div style="font-weight:700;font-size:15px">${t.name||t.title||"Package"}</div>
                  ${t.description?`<div style="color:rgba(255,255,255,0.5);font-size:13px;margin-top:2px">${t.description}</div>`:""}
                </div>
                <div style="font-family:var(--font-display,'Manrope',sans-serif);font-size:18px;font-weight:900;color:var(--brand,#FF5C00);white-space:nowrap;letter-spacing:-0.02em">${t.price?`${t.currency||""} ${t.price}`.trim():""}</div>
              </div>`).join("")}
            </div>
          </div>`:""}
