...
import { g as getLocale, t, $ as $$Base } from './Base_X9W0huPu.mjs';
...
function buildFeaturedCard(a) {
  const authorName = a.author || (window.__BRAND__?.name || 'TrainedBy') + ' Trainer';
  const initials = authorName.split(' ').map(w=>w[0]).slice(0,2).join('').toUpperCase();
  const avatarHtml = \`<div class="article-meta-avatar-initials">\${initials}</div>\`;
...
function buildArticleCard(a) {
  const authorName = a.author || 'Trainer';
  const imgHtml = a.cover_url
...
const LOCALE_SAMPLES = sampleArticles;
