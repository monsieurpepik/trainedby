import { g as getMarket, $ as $$Base } from './Base_DaVXhZTk.mjs';
import { c as createComponent } from './astro-component_e3G5FwE5.mjs';
import { h as renderComponent, r as renderTemplate, m as maybeRenderHead } from './ssr-function_CjOLPKa-.mjs';
...
function trainerCardHTML(t) {
  ...
  <a href="/\${slug}" class="trainer-card">
    ...
    <div class="card-body">
      <div class="card-top">
        ...
        <div class="card-info">
          ...
          <div class="card-title">\${t.title || 'Personal Trainer'}\${t.city ? ' · ' + t.city : ''}</div>
        </div>
      </div>
      \${starsHtml}
    ...
    <div class="card-footer">
      <div class="card-price">\${t.min_price ? \`\${window.__CURRENCY__ || ''} \${t.min_price.toLocaleString()} <span>/ session</span>\`.trim() : '<span>Contact for price</span>'}</div>
...
loadTrainers();
<\/script> `])), maybeRenderHead(), brandName, market.certificationBody.endsWith("Verified") ? market.certificationBody + " Trainers" : market.certificationBody + " Verified Trainers", market.i18nLocale === "fr" ? "Parcourez les coachs certifiés. Filtrez par spécialité, lieu et mode d'entraînement." : market.i18nLocale === "it" ? "Sfoglia i trainer certificati. Filtra per specialità, luogo e modalità di allenamento." : market.i18nLocale === "es" ? "Explora entrenadores certificados. Filtra por especialidad, ubicación y modalidad." : `Browse ${market.certificationBody} verified trainers. Filter by specialty, location, price and training style.`, Array(6).fill(0).map(() => `
