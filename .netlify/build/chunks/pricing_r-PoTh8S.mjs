...
import { h as renderComponent, r as renderTemplate, k as Fragment, j as defineScriptVars, u as unescapeHTML, m as maybeRenderHead } from './ssr-function_BO-eb4Zn.mjs';
import { g as getLocale, t, $ as $$Base, a as getBrand } from './Base_D7YaUbjW.mjs';
import { $ as $$Nav } from './Nav_DW9al5qJ.mjs';
...
async function startCheckout(plan) {
  ...
  try {
    ...
    if (!verifyRes.ok || !verifyData.trainer) { showError('Session expired. Please log in again.'); setTimeout(() => { location.href = '/join'; }, 1500); return; }
    ...
    if (!res.ok) { showError(data.error || 'Could not start checkout.'); return; }
    ...
    if (err && err.message && err.message.includes('razorpay')) {
      showError('Payment provider unavailable. Please try again or contact support.');
    } else {
      showError('Could not start checkout. Please try again.');
    }
  }
